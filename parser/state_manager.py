...
from parser.config import OPEN_THRESHOLD, CLOSE_THRESHOLD, DEEPEN_TRIGGER, MIN_VOLUME_24H_USD
...
async def run_state_manager(in_queue: asyncio.Queue, out_queue: asyncio.Queue) -> None:
    ...
    try:
        while True:
            ...
            if symbol not in open_alerts and abs(spread_pct) >= OPEN_THRESHOLD:
            ...
            elif symbol in open_alerts and abs(spread_pct) < CLOSE_THRESHOLD:
            ...
            elif symbol in open_alerts and abs(spread_pct) >= open_alerts[symbol]['peak_pct'] * DEEPEN_TRIGGER:
